function CTMC = CTMC_cold_standby(n, k, working_failure_rate, cold_standby_switching_rate, repair_rate)
    ...
    for i = 1:(n-k+1)
        ...
        State_space(i, 2) = n - k - j; % number of cold standby components
    ...
    for i = (n-k+2):(2*n-2*k+3)
        ...
        State_space(i, 2) = n - k + 1 - j; % number of cold standby components
        State_space(i, 3) = j; % number of failed components
        j = j + 1;
    end
    ...
    for i = 1:(2*n - 2*k + 3)
        for j = 1:(2*n - 2*k + 3)
            ...
            elseif State_space(i,2) - State_space(j,2) == 1 && State_space(j,1) - State_space(i,1) == 1
                % Switch
                CTMC(i, j) = cold_standby_switching_rate;
